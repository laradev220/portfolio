...
export default function Contact() {
  return (
    <section id="contact" className="py-24 px-6 bg-tokyo-surface/30">
      <div className="max-w-4xl mx-auto">
        ...
        <ScrollReveal delay={0.1}>
          <div className="text-center mb-10">
            <p className="text-tokyo-muted text-lg mb-4">
            ...
            <ScrollReveal key={method.label} delay={0.2 + index * 0.1}>
              {method.href ? (
                <motion.a
                  ...
                  className={`bg-tokyo-surface border border-tokyo-border rounded-lg p-6 text-center block ${method.hoverColor} transition-colors group`}
                ...
                >
                  ...
                  <p className="text-tokyo-text font-medium">{method.label}</p>
                  <p className="text-tokyo-muted text-sm group-hover:text-tokyo-text transition-colors">{method.value}</p>
                </motion.a>
              ) : (
                <motion.div
                  className="bg-tokyo-surface border border-tokyo-border rounded-lg p-6 text-center"
                  whileHover={{ scale: 1.03, y: -4 }}
                >
                  <motion.div
                    className={`mx-auto mb-3 ${method.color}`}
                  ...
                  <p className="text-tokyo-text font-medium">{method.label}</p>
                  <p className="text-tokyo-muted text-sm">{method.value}</p>
                </motion.div>
              )}
            </ScrollReveal>
          ))}
